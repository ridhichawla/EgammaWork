...
class SimpleElectronNtupler : public edm::EDAnalyzer {
  ...
  private:
    ...
    std::vector<double> pt_sEle_mc;
    std::vector<double> eta_sEle_mc;
    std::vector<double> phi_sEle_mc;

    std::vector<double> pt_dEle_data;
    std::vector<double> eta_dEle_data;
    std::vector<double> phi_dEle_data;

    std::vector<double> pt_sMuon;
    std::vector<double> eta_sMuon;
    std::vector<double> phi_sMuon;
  ...
  //mvaCategoriesMapToken_(consumes<edm::ValueMap<int> >(iConfig.getParameter<edm::InputTag>("mvaCategoriesMap")))
{
  ...
  electronTree_->Branch("pt_sEle_mc"    ,  &pt_sEle_mc    );
  electronTree_->Branch("eta_sEle_mc"   ,  &eta_sEle_mc   );
  electronTree_->Branch("phi_sEle_mc"   ,  &phi_sEle_mc   );
  electronTree_->Branch("pt_dEle_data"    ,  &pt_dEle_data    );
  electronTree_->Branch("eta_dEle_data"   ,  &eta_dEle_data   );
  electronTree_->Branch("phi_dEle_data"   ,  &phi_dEle_data   );
  electronTree_->Branch("pt_sMuon", &pt_sMuon);
  electronTree_->Branch("eta_sMuon", &eta_sMuon);
  electronTree_->Branch("phi_sMuon", &phi_sMuon);
...
SimpleElectronNtupler::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  std::string SMFilter("hltL3crIsoL1sMu16L1f0L2f10QL3f20QL3trkIsoFiltered0p09");
  //hltL3MuonCandidates
/*  
     bool trigResult = false;
...
*/
	 ...
	 if((SEMCFilter.compare(obj.filterLabels()[j]))==0){
	   //probePass = true;
	   pt_sEle_mc.push_back(obj.pt());
	   eta_sEle_mc.push_back(obj.eta());
	   phi_sEle_mc.push_back(obj.phi());
	 }

	 if((SEDataFilter.compare(obj.filterLabels()[j]))==0){
	   //probePass = true;
	   pt_dEle_data.push_back(obj.pt());
	   eta_dEle_data.push_back(obj.eta());
	   phi_dEle_data.push_back(obj.phi());
	 }

	 if((SMFilter.compare(obj.filterLabels()[j]))==0){
	   //probePass = true;
	   pt_sMuon.push_back(obj.pt());
	   eta_sMuon.push_back(obj.eta());
	   phi_sMuon.push_back(obj.phi());
	 }
	 ...
	 double_electron_triggers_in_run.push_back(hltname);
       ...
       if(found_singleMuon !=string::npos){
	 single_muon_triggers_in_run.push_back(hltname);
       }
     }
