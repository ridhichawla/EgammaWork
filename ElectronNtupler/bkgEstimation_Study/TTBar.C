...
//cout<<"#"<<endl;
    if(nMuons>=1){
    //if(isGLBmuon && isPFmuon){

      for(int l=0;l<nMuons;l++){

	//cout<<"muon pt: "<<ptMuon->at(l)<<"   "<<"muon eta: "<<etaMuon->at(l)<<endl;
	//cout<<"norm chi2: "<<normchi2Muon->at(l)<<endl;
	//cout<<"1"<<endl;

	if(fabs(etaMuon->at(l)) < 2.4 && ptMuon->at(l) > 20.){
	//cout<<"2"<<endl;
	//cout<<"norm chi2: "<<normchi2Muon->at(l)<<endl;
	//cout<<"muon hits: "<<nhitsMuon->at(l)<<endl;
	  //if(normchi2Muon->at(l) < 10. && nhitsMuon->at(l) > 0. && nMatchesMuon->at(l) > 1. && dxyVtxMuon->at(l) < 0.2 && dzVtxMuon->at(l) < 0.5 && npixelHitsMuon->at(l) > 0. && trackerLayersMuon->at(l) > 0.)
	  if(isTight->at(l)){
	  //cout<<"3"<<endl;
