...
void apply2FR(int index) {
  ...
  for (unsigned int jentry=0; jentry < nentries; jentry++) {
	...
	if(ptcut){
	  ...
	  if(etaSC->at(failingElec[0]) < 1.4442 && etaSC->at(failingElec[1]) < 1.4442) corr = 1.6139;
	  else if((etaSC->at(failingElec[0]) < 1.4442 && etaSC->at(failingElec[1]) > 1.566) || (etaSC->at(failingElec[0]) > 1.566 && etaSC->at(failingElec[1]) < 1.4442)) corr = 2.4759;
	  else if(etaSC->at(failingElec[0]) > 1.566 && etaSC->at(failingElec[1]) > 1.566) corr = 1.4054;
	  
	  float randomnumber = gRandom->Gaus(0.0,corr);
	  corrmass = mass + randomnumber;

	  //cout<<"random = "<<randomnumber<<"   "<<"corrmass = "<<corrmass<<endl;
	  ...
	  if( sign < 0 ) {
	    ...
	    if( mass > 15 && mass < 3000) {
	      ...
	      histDijet1->Fill(corrmass, weight_template);
		...
		histDijet1_barrel->Fill(corrmass, weight_template);
		...
		histDijet1_barend->Fill(corrmass, weight_template);
		...
		histDijet1_endcap->Fill(corrmass, weight_template);
