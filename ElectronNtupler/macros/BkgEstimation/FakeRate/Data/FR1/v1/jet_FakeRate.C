...
void jet_FakeRate() {
  ...
  TFile *file = new TFile("fakeRate1_Photon.root", "recreate");
  ...
  for (unsigned int jentry=0; jentry < nentries; jentry++) {
	...
	//if(fabs(etaSC->at(index[j])) < 2.5 && expectedMissingInnerHits->at(index[j]) == 0)
	if(passKin && expectedMissingInnerHits->at(index[j]) == 0){
