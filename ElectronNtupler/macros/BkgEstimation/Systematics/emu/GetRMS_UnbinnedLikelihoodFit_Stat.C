...
void GetRMS_UnbinnedLikelihoodFit_Stat()
{

  TFile *f1 = TFile::Open("ROOTFile_StatUnc_BkgEst.root");
  TFile *f2 = new TFile("RelUnc_BkgEst_StatUnc_UnbinnedFit.root","RECREATE");
  ...
  for(Int_t i=40; i<41; i++)
  {
    ...
    TString CanvasName = "c_RelDiff_BkgEst_Stat_" + TString::Format("_%d", i_bin) + "bin";
...
void Set_FitInitValues( Double_t BinCenter, Double_t &RangeMax, Double_t &Sigma_Init )
{
  ...
  else if( BinCenter > 700 )
  {
    RangeMax = 0.5;
    Sigma_Init = 0.02;
  }
  else if( BinCenter > 243 )
  {
    RangeMax = 0.05;
    Sigma_Init = 0.01;
  }
