...
void subtractEWK_2L_signal() {
  ...
  for(unsigned int jentry = 0; jentry < nsample; ++jentry) {
    ...
    file[jentry] = new TFile(Form("2Loose/DYEE_M%dto%d.root",mass[jentry],mass[jentry+1]),"RECREATE");
    //file[jentry] = new TFile(Form("Test/DYEE_M%dto%d.root",mass[jentry],mass[jentry+1]),"RECREATE");
