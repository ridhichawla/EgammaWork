...
void jet_FakeRate() {
  ...
  TFile f1("/eos/cms/store/group/phys_higgs/cmshww/arun/DYAnalysis_76X_Calibrated/Data/Photon_2015.root");
  ...
  T1->SetBranchStatus("passLooseId", 1);
  ...
  for (unsigned int jentry=0; jentry < nentries; jentry++) {
    ...
    for(unsigned int j=0;j<ptElec->size();j++){
      ...
      looseId = passLooseId->at(index[j]);
      passKin = (ptElec->at(index[j]) > 10. && fabs(etaSC->at(index[j])) < 2.5 && !(fabs(etaSC->at(index[j])) > 1.4442 && fabs(etaSC->at(index[j])) < 1.566));

      if(looseId && passKin && expectedMissingInnerHits->at(index[j])){
    ...
    for(unsigned int k=0;k<elePt.size();k++){
      if(elePt.at(k)  > 10000.) continue;

      if(elePt.at(k) > 10000.) cout<<"Check"<<endl;

      denPt->Fill(elePt.at(k),prescalePhoton);
      denEta->Fill(eleEta.at(k),prescalePhoton);

      if(fabs(scEta.at(k)) < 1.4442) denPt_BRL->Fill(elePt.at(k),prescalePhoton);
      else if(fabs(scEta.at(k)) > 1.566 && fabs(scEta.at(k)) < 2.5) denPt_ECAP->Fill(elePt.at(k),prescalePhoton);

      passId = eleMediumId.at(k);
      if(passId){

	numPt->Fill(elePt.at(k),prescalePhoton);
	numEta->Fill(eleEta.at(k),prescalePhoton);

	if(fabs(scEta.at(k)) < 1.4442) numPt_BRL->Fill(elePt.at(k),prescalePhoton);
	else if(fabs(scEta.at(k)) > 1.566 && fabs(scEta.at(k)) < 2.5) numPt_ECAP->Fill(elePt.at(k),prescalePhoton);
