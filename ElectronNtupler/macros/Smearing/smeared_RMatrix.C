...
void smeared_RMatrix() {
  ...
  for(unsigned int jentry = 0; jentry < nsample; ++jentry) {
    ...
    int mediumId;
    double massGPre;
    double dR1, dR2, dR3, dR4, dR5, dR6, dR7, dR8, dR10, dR11, dR12;
    double events;
    ...
    for(Int_t i_map=0; i_map<nSmearMap; i_map++) {
      ...
      tree[i_map]->Branch("isReco4", &isReco4, "isReco4/B");
      ...
      tree[i_map]->Branch("BB4", &BB4, "BB4/B"); tree[i_map]->Branch("BE4", &BE4, "BE4/B"); tree[i_map]->Branch("EE4", &EE4, "EE4/B");
	...
	if(Ele23_WPLoose){
	  ...
	  for(int k=0;k<ptElec->size();k++){
		...
		newelePt4.push_back(ptElec->at(k)*shift4);
	  ...
	  // Sorting Reco4 level
	  int index4[newelePt4.size()];
	  float pt4[newelePt4.size()];

	  for(unsigned int el4=0; el4<newelePt4.size(); el4++)
	  { 
	    pt4[el4]=newelePt4.at(el4);
	  }
	  ...
	  if(newelePt4.size() == 2){
	    int countTrig4 = 0;
	    for(unsigned int q = 0; q < eta_Ele23->size(); q++){
	      dR9  = deltaR(neweleEta.at(index4[0]), newelePhi.at(index4[0]), eta_Ele23->at(q), phi_Ele23->at(q));
	      dR10 = deltaR(neweleEta.at(index4[1]), newelePhi.at(index4[1]), eta_Ele23->at(q), phi_Ele23->at(q));
	      if(dR9 < 0.1 || dR10 < 0.1) countTrig4++;
	    } // eta_Ele23

	    if(newelePt4.at(index4[0]) > 30. && newelePt4.at(index4[1]) > 10. && countTrig4 != 0){
	      isReco4 = true;
	      ele41.SetPtEtaPhiE(newelePt4.at(index4[0]),neweleEta.at(index4[0]),newelePhi.at(index4[0]),neweleEnr4.at(index4[0]));
	      ele42.SetPtEtaPhiE(newelePt4.at(index4[1]),neweleEta.at(index4[1]),newelePhi.at(index4[1]),neweleEnr4.at(index4[1]));

	      dielectron4=ele41+ele42;
	      massReco4=dielectron4.M();

	      if(fabs(newscEta.at(index4[0])) < 1.4442 && fabs(newscEta.at(index4[1])) < 1.4442) BB4 = true;
	      if((fabs(newscEta.at(index4[0])) < 1.4442 && fabs(newscEta.at(index4[1])) > 1.566) || (fabs(newscEta.at(index4[0])) > 1.566 && fabs(newscEta.at(index4[1])) < 1.4442)) BE4 = true;
	      if(fabs(newscEta.at(index4[0])) > 1.566 && fabs(newscEta.at(index4[1])) > 1.566) EE4 = true;
