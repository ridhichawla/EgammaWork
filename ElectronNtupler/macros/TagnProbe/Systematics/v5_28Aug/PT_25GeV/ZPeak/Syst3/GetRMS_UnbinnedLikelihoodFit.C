...
void GetRMS_UnbinnedLikelihoodFit()
{
	...
	Double_t MassBinEdges[nMassBin+1] = {}; // -- Merging high-mass bins -- //
...
TTree* makeTTree(TFile *f_input, Int_t i_bin) 
{
	...
	f_input->cd();
	TH1D *h_DiffXsec_CV = (TH1D*)f_input->Get(HistName+"_CentralValue")->Clone();
	Double_t value_CV = h_DiffXsec_CV->GetBinContent(i_bin);

	for(Int_t i=0; i<nMap; i++)
	{
		TH1D *h_DiffXsec_Smeared = (TH1D*)f_input->Get( HistName+"_"+TString::Format("%d", i) );
