...
void GetRMS_UnbinnedLikelihoodFit()
{
	...
	TFile *f1 = TFile::Open("ROOTFile_Syst1Unc_EffCorr.root");
	TFile *f2 = new TFile("RelUnc_EffSF_Syst1Unc_UnbinnedFit.root","RECREATE");
...
TTree* makeTTree(TFile *f_input, Int_t i_bin) 
{
	...
	TString HistName = "h_mass_EffPass_Corr"; // h_DiffXsec
	TString FileName = f_input->GetName();
	if( FileName.Contains("FpoF") )
		HistName = "h_FpoF_DiffXsec";

	f_input->cd();
	TH1D *h_EffSF_CV = (TH1D*)f_input->Get(HistName+"_CentralValue")->Clone();
	Double_t value_CV = h_EffSF_CV->GetBinContent(i_bin);

	for(Int_t i=0; i<nMap; i++)
	{
		TH1D *h_EffSF_Smeared = (TH1D*)f_input->Get( HistName+"_"+TString::Format("%d", i) );
		Double_t value_Smeared = h_EffSF_Smeared->GetBinContent(i_bin);
