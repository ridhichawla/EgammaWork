...
void FinalEffSingle()
{
  ...
  TFile *file = TFile::Open("ROOTFile_EffSF_Trig.root","recreate");
  ...
  float eff_mc_Trig, eff_data_Trig, eff_SF;
  int binX;
  int binY;
  ...
  for(int i=0; i<nentries; i++)
  {
    ...
    eff_SF   = eff_data_Trig/eff_mc_Trig;
    if(eff_data_Trig == 0 || eff_mc_Trig == 0) cout<<"entry: "<<i<<"   "<<eff_data_Trig<<"   "<<eff_mc_Trig<<endl;
