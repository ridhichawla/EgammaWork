...
void FinalEffSingle()
{
  ...
  TFile *file = TFile::Open("ROOTFile_EffSF_ID.root","recreate");
  ...
  float eff_mc_ID, eff_data_ID, eff_SF;
  int binX;
  int binY;
  ...
  for(int i=0; i<nentries; i++)
  {
    ...
    LeptonScaleLookup a("Medium_Stat.root");
    t->GetEntry(i);

    eff_mc_ID   = a.GetExpectedEfficiency(Ele1Eta,Ele1PT,Ele2Eta,Ele2PT,mc);
    eff_data_ID = a.GetExpectedEfficiency(Ele1Eta,Ele1PT,Ele2Eta,Ele2PT,data);

    eff_SF   = eff_data_ID/eff_mc_ID;
    if(eff_data_ID == 0 || eff_mc_ID == 0) cout<<"entry: "<<i<<"   "<<eff_data_ID<<"   "<<eff_mc_ID<<endl;
