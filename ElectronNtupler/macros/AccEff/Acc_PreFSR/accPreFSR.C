...
void accPreFSR() {
  ...
  for(unsigned int j = 0; j < nsample; ++j ) {
    ...
    file[j] = new TFile(Form("Test6/DYEE_M%dto%d.root",mass[j],mass[j+1]),"RECREATE");
    ...
    vector <double> gPreFSR_Px; vector <double> gPreFSR_Py; vector <double> gPreFSR_Pz;
	...
	pt2[b]=genPostFSR_Pt->at(b);
      ...
      gPreFSR_Px.clear(); gPreFSR_Py.clear(); gPreFSR_Pz.clear();
	...
	int index3[gPreFSR_Pt.size()];
	float pt3[gPreFSR_Pt.size()];

	for(unsigned int c=0; c<gPreFSR_Pt.size(); c++)
	{                       
	  pt3[c]=gPreFSR_Pt.at(c);
	}                                     
	int size3 = sizeof(pt3)/sizeof(pt3[0]);     
	TMath::Sort(size3,pt3,index3,true);

	double E7 = sqrt(gPreFSR_Px.at(index3[0])*gPreFSR_Px.at(index3[0]) + gPreFSR_Py.at(index3[0])*gPreFSR_Py.at(index3[0]) + gPreFSR_Pz.at(index3[0])*gPreFSR_Pz.at(index3[0]) + 0);
	double E8 = sqrt(gPreFSR_Px.at(index3[1])*gPreFSR_Px.at(index3[1]) + gPreFSR_Py.at(index3[1])*gPreFSR_Py.at(index3[1]) + gPreFSR_Pz.at(index3[1])*gPreFSR_Pz.at(index3[1]) + 0);

	pre1.SetPxPyPzE(gPreFSR_Px.at(index3[0]),gPreFSR_Py.at(index3[0]),gPreFSR_Pz.at(index3[0]),E7);
	pre2.SetPxPyPzE(gPreFSR_Px.at(index3[1]),gPreFSR_Py.at(index3[1]),gPreFSR_Pz.at(index3[1]),E8);
	...
	Flag_PassAcc = isPassAccCondition_GenLepton_ECALGAP(gPreFSR_Pt.at(index3[0]),gPreFSR_Eta.at(index3[0]),gPreFSR_Pt.at(index3[1]),gPreFSR_Eta.at(index3[1]));
	...
	h_Pt_Lead_NoAcc_PreFSR->Fill(gPreFSR_Pt.at(index3[0]),theWeight*lumiWeight);
	h_Pt_SubLead_NoAcc_PreFSR->Fill(gPreFSR_Pt.at(index3[1]),theWeight*lumiWeight);
	h_Eta_Lead_NoAcc_PreFSR->Fill(gPreFSR_Eta.at(index3[0]),theWeight*lumiWeight);
	h_Eta_SubLead_NoAcc_PreFSR->Fill(gPreFSR_Eta.at(index3[1]),theWeight*lumiWeight);

	if(Flag_PassAcc) {
	  h_Pt_Lead_Acc_PreFSR->Fill(gPreFSR_Pt.at(index3[0]),theWeight*lumiWeight);
	  h_Pt_SubLead_Acc_PreFSR->Fill(gPreFSR_Pt.at(index3[1]),theWeight*lumiWeight);
	  h_Eta_Lead_Acc_PreFSR->Fill(gPreFSR_Eta.at(index3[0]),theWeight*lumiWeight);
	  h_Eta_SubLead_Acc_PreFSR->Fill(gPreFSR_Eta.at(index3[1]),theWeight*lumiWeight);
