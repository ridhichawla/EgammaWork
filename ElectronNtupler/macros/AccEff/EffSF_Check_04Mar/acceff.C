...
Bool_t isPassAccCondition_GenLepton_ECALGAP(double leadpt, double leadeta, double subleadpt, double subleadeta)
{
  ...
  if( leadpt > 25. && fabs(leadeta) < 2.5 && !( fabs(leadeta) > 1.4442 && fabs(leadeta) < 1.566 ) &&
      subleadpt  > 25.  && fabs(subleadeta) < 2.5 && !( fabs(subleadeta) > 1.4442 && fabs(subleadeta) < 1.566 ) )
    isPassAcc = 1;
...
void acceff() {
  ...
  for(unsigned int j = 1; j < 2; ++j ) {
    ...
    file[j] = new TFile(Form("DYEE_M%dtoInf.root",mass[j]),"RECREATE");
